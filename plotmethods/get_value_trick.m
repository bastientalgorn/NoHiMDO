...
%-------------------------------------------------------------------------------------%
function v = get_value_trick(pb_name,NO,NI,optim_method,field)

dir = './matfiles/';

% Name of the file where to fetch the matrix M
mat_file = [dir pb_name '_no' num2str(NO) '_ni' num2str(NI) '_s' optim_method '.mat'];
...
elseif findstr('q0.90',field)
    j = j+9;
elseif findstr('dm',field)
    j = 19;
elseif findstr('q3',field)
    j = 20;
elseif findstr('q6',field)
    j = 21;
end
...
if findstr('obj',field)
    fstar = get_f_star(pb_name);
    v = abs(v-fstar)/abs(fstar);
end
